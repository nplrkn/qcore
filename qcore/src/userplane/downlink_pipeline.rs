...
use super::{
    GTP_MESSAGE_TYPE_GPU, GTPU_PORT, INNER_PACKET_OFFSET, IPV4_HEADER_LEN, MAX_UES,
    PDCP_HEADER_LEN, SDAP_HEADER_LEN,
};
use anyhow::Result;
use async_std::{
    io::ReadExt,
...
impl DownlinkPipeline {
    ...
    async fn handle_next_downlink_packet(&self, buf: &mut [u8; 2000]) -> Result<()> {
        let bytes_read = self
            .n6_tun_device
            .reader()
            .read(&mut buf[INNER_PACKET_OFFSET..2000])
            .await?;
        ...
        let ip_header = &buf[INNER_PACKET_OFFSET..INNER_PACKET_OFFSET + IPV4_HEADER_LEN];
        // TODO: check IP type
        ...
        // buf[4..8] already done earlier.

        // ---- PDCP Data PDU for DRB with 12 bit PDCP SN ----
        buf[8] = 0b1_0_0_0_0000 | (((sn & 0x0f00) >> 8) as u8); // D/C, R,R,R, SN
        buf[9] = (sn & 0xff) as u8; // SN

        // ---- SDAP DOWNLINK DATA PDU ----
        buf[10] = 0b0_0_000001; // RDI, RQI, QFI - see TS37.324

        // buf[11..bytes_read] has the original IP packet we received.

        let du_ip = IpAddr::try_from(du_ip)?;
        self.f1u_socket
            .send_to(
                &buf[0..(bytes_read + INNER_PACKET_OFFSET)],
