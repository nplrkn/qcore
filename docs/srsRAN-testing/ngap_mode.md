...
```sh
cd ~/qcore
RUST_LOG=debug cargo run -- --local-ip 127.0.0.1 --no-dhcp --sim-cred-file docs/srsRAN-testing/srs-sim.toml --userplane-stats
```
...
```sh
cd && sudo tcpdump -w srsran_test_ngap_cu_du.pcap -i any sctp or port 2152 or host 10.255.0.2
```
...
```sh
cd ~/qcore
RUST_LOG=debug cargo run -- --local-ip 127.0.0.3  --no-dhcp --sim-cred-file docs/srsRAN-testing/srs-sim.toml
```
