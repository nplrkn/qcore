...
```sh
cd ~/qcore
RUST_LOG=debug cargo run -- --mcc 001 --mnc 01 --local-ip 127.0.0.1 --f1u-interface-name lo --sim-cred-file ~/sims.toml --ngap-mode
```
...
```sh
cd ~/qcore
RUST_LOG=debug cargo run -- --mcc 001 --mnc 01 --local-ip 127.0.0.3 --f1u-interface-name lo --sim-cred-file ~/sims.toml --ngap-mode
```
