...
There is a single-instance UE task ("UE message handler") that gets all UE related messages delivered to it over
a Rust channel.  This task initiates UE procedures and, while a procedure is running, allows it to wait for the next UE message.
...
In this section, we study what procedures actually ought to be able to run in parallel.  For now it just considers the NAS layer, and just the subset of procedures that are implemented in QCore.
